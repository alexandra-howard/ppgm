...
ppgm_bm <- ppgm(occurrences, trees = vigtrees, bounds = bmbounds, control = contr, 
                    which.biovars = c(1,4), model = "BM", plot.TraitGram = TRUE, path = "BM")

```

This should produce a figure in your working directory called "BMbio1.pdf" and "BMbio4.pdf".
...
knitr::include_graphics("./BMbio4.pdf")
```
...
ppgm_ou <- ppgm(occurrences, trees = vigtrees, bounds = oubounds, control = contr, 
                    which.biovars = c(1,4), model = "OU", plot.TraitGram = TRUE, path = "OU")
...
knitr::include_graphics("./OUbio4.pdf")
```
...
```{r run ppgmMESS}
mess <- ppgmMESS(cem_min,cem_max,ppgm_bm$node_est,tree=vigtrees,timeslice=15, which.biovars=c(1,4),which.plot="mess")
```
...
```{r image-ppgm-15mess, echo = FALSE, message=FALSE, fig.align='center', fig.cap='Traitgram showing Temperature Seasonality', out.width="75%", fig.pos='H'}
knitr::include_graphics("./MESS15multi.pdf")
```
...
ppgm_bmfossil <- ppgm(occurrences, trees = vigtrees, fossils = vigfossils, bounds = bmbounds,
                      control = contr, which.biovars = c(1,4), model = "BM", plot.TraitGram = TRUE,
                      path = "BMfossil", permut = 5)
```

We can compare the results with the extant only ppgm

Also create a MESS map.

```{r format fossil output for ppgmMESS}
cem_min <- cbind(ppgm_bmfossil$cem[,1], ppgm_bmfossil$cem[,2])
cem_max <- cbind(ppgm_bmfossil$cem[,5], ppgm_bmfossil$cem[,6])
rownames(cem_min) <- rownames(cem_max) <- rownames(ppgm_bmfossil$cem)
```


```{r run fossil ppgmMESS}
mess_fossil <- ppgmMESS(cem_min, cem_max, ppgm_bmfossil$node_est, tree=vigtrees, timeslice=15,
                        which.biovars = c(1,4), which.plot = "mess", fossils = scel_fossils,
                        path = "fossil")
```


As you can see the MESS map using fossils is very different, suggesting climatic tolerances for *Sceloporus* much more north than using extant occurrences alone.